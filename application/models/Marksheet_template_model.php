...
class Marksheet_template_model extends MY_Model
{
    ...
    public function save($data)
    {
        $arrayLive = array(
            ...
            'subjects_table' => isset($_POST['subjects_table']) ? 1 : 0,
    ...
    public function tagsList()
    {
        ...
        $arrayTags[] = '{jr_teacher_comment}';
        $arrayTags[] = '{jr_head_teacher_comment}';
        return $arrayTags;
    }
    ...
    public function tagsReplace($studentData = '', $template = '', $extendsData = [], $header_content = '')
    {
        ...
        foreach ($tags as $tag) {
            ...
            } else if ($field == 'jr_teacher_comment') {
                if (!empty($extendsData['jr_teacher_comment'])) {
                    $body = str_replace($tag, nl2br(htmlspecialchars($extendsData['jr_teacher_comment'])), $body);
                } else {
                    $body = str_replace($tag, "", $body);
                }
            } else if ($field == 'jr_head_teacher_comment') {
                if (!empty($extendsData['jr_head_teacher_comment'])) {
                    $body = str_replace($tag, nl2br(htmlspecialchars($extendsData['jr_head_teacher_comment'])), $body);
                } else {
                    $body = str_replace($tag, "", $body);
                }
            } else {
