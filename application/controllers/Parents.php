...
class Parents extends Admin_Controller
{
    ...
    // bulk disable accounts functionality for parents
    public function bulk_disable_accounts()
    {
        ...
        if (isset($_POST['disable_accounts'])) {
            ...
            $parentslist = $this->input->post('bulk_disable_operations');
            if (isset($parentslist)) {
                foreach ($parentslist as $id) {
                    $this->db->where(array('role' => 6, 'user_id' => $id));
            ...
            redirect(base_url('parents/bulk_disable_accounts'));
        }
        $this->data['parentslist'] = $this->parents_model->getParentList('', 1);
        $this->data['title'] = translate('bulk_disable_accounts');
        $this->data['sub_page'] = 'parents/bulk_disable_accounts';
        $this->data['main_menu'] = 'parents';
        $this->load->view('layout/index', $this->data);
    }
