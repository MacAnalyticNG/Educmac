...
class Employee extends Admin_Controller
{
    ...
    // bulk disable accounts functionality
    public function bulk_disable_accounts()
    {
        ...
        if (isset($_POST['search'])) {
            $branchID = $this->application_model->get_branch_id();
            $role = $this->input->post('staff_role');
            $this->data['stafflist'] = $this->employee_model->getStaffList($branchID, $role, 1);
        }
        ...
        if (isset($_POST['disable_accounts'])) {
            ...
            $stafflist = $this->input->post('bulk_disable_operations');
            if (isset($stafflist)) {
                foreach ($stafflist as $id) {
                    $this->db->where('user_id', $id);
                    $this->db->where_not_in('role', array(1, 6, 7));
            ...
            redirect(base_url('employee/bulk_disable_accounts'));
        }
        $this->data['title'] = translate('bulk_disable_accounts');
        $this->data['sub_page'] = 'employee/bulk_disable_accounts';
        $this->data['main_menu'] = 'employee';
        $this->load->view('layout/index', $this->data);
    }
