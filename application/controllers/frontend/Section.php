    ...
    public function exam_results()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                // Save section-template mappings
                $section_templates = $this->input->post('section_template');
                if (!empty($section_templates)) {
                    foreach ($section_templates as $section_id => $template_id) {
                        // If template_id is 0 (Default Template), delete any existing mapping
                        if ($template_id == 0) {
                            $this->db->where(array('branch_id' => $branchID, 'section_id' => $section_id));
                            $this->db->delete('section_marksheet_template');
                        } else {
                            // For valid template IDs, insert or update mapping
                            $mappingData = array(
                                'branch_id' => $branchID,
                            ...
                            $exists = $this->db->where(array('branch_id' => $branchID, 'section_id' => $section_id))
                            ...
                            if ($exists) {
                                ...
                                $this->db->where(array('branch_id' => $branchID, 'section_id' => $section_id));
                                $this->db->update('section_marksheet_template', array('template_id' => $template_id));
                            } else {
                                // Insert new mapping
                                $this->db->insert('section_marksheet_template', $mappingData);
                            }
                        }
                    }
                }
