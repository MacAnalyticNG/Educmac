...
class Home extends Frontend_Controller
{
    ...
    public function examResultsPrintFn()
    {
        ...
        if ($_POST) {
            ...
            if ($this->form_validation->run() == true) {
                ...
                // CHECK 1: Parent Account Status - ALWAYS CHECK FIRST
                if (!empty($userID['parent_id'])) {
                    $parent_status = $this->db->select('login_credential.active')
                        ->from('login_credential')
                        ->where('login_credential.user_id', $userID['parent_id'])
                        ->where('login_credential.role', 6)
                        ->get()
                        ->row();

                    if ($parent_status && $parent_status->active == 0) {
                        $array = array('status' => '0', 'error' => "You don't have access to this result. Kindly contact the school admin.");
                        echo json_encode($array);
                        exit();
                    }
                }

                // CHECK 2: Fee Payment Threshold
                $fee_check = can_view_exam_results($userID['enroll_id'], $examID, $sessionID);
                if (!$fee_check['allowed']) {
                    $array = array('status' => '0', 'error' => $fee_check['message']);
                    echo json_encode($array);
                    exit();
                }
                ...
                $this->data['class_id'] = $userID['class_id'];
                $this->data['section_id'] = $userID['section_id'];
                $this->data['sessionID'] = $sessionID;
                $this->data['branchID'] = $userID['branch_id'];
                $this->data['print_date'] = date('Y-m-d');
    ...
    public function examResultsPDF()
    {
        $this->load->model('exam_model');
        $this->load->model('marksheet_template_model');
        $this->load->model('skills_model');
        if ($_POST) {
            ...
            if (empty($userID)) {
                show_error('Register No Not Found.');
                exit();
            }
            ...
            $html = $this->load->view('home/reportCardWithTemplate', $this->data, true);

            $this->load->library('html2pdf');
            $this->html2pdf->mpdf->WriteHTML($html);
            $this->html2pdf->mpdf->Output('exam_result_' . $registerNo . '.pdf', 'D');
        }
    }
