...
class Student extends Admin_Controller
{
    ...
    // bulk disable accounts functionality for students
    public function bulk_disable_accounts()
    {
        // check access permission
        ...
        if (isset($_POST['search'])) {
            $classID = $this->input->post('class_id');
            $sectionID = $this->input->post('section_id');
            $this->data['students'] = $this->application_model->getStudentListByClassSection($classID, $sectionID, $branchID, false);
        }

        if (isset($_POST['disable_accounts'])) {
            if (!get_permission('student', 'is_edit')) {
                access_denied();
            }
            $studentlist = $this->input->post('bulk_disable_operations');
            if (isset($studentlist)) {
                ...
                set_alert('success', translate('accounts_have_been_disabled_successfully'));
            } else {
                set_alert('error', 'Please select at least one account to disable');
            }
            redirect(base_url('student/bulk_disable_accounts'));
        }
        $this->data['branch_id'] = $branchID;
        $this->data['title'] = translate('bulk_disable_accounts');
        $this->data['sub_page'] = 'student/bulk_disable_accounts';
        $this->data['main_menu'] = 'student';
        $this->load->view('layout/index', $this->data);
    }
