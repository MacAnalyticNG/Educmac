...
<div class="container px-md-0 main-container">
    ...
    <div class="box2 form-box">
        ...
        <button type="submit" class="btn btn-1" data-loading-text="<i class='fas fa-spinner fa-spin'></i> Processing"><i class="fas fa-eye"></i> <?= translate('preview') ?></button>
    </div>
    <?php echo form_close(); ?>
    <div class="row">
        <div class="col-md-12">
            <div id="card_holder" style="display: none;">
                <div class="box2 form-box">
                    <button type="button" class="btn btn-success" id="downloadPDF"><i class="fas fa-download"></i> Download PDF</button>
                    <div id="card"></div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        $('#downloadPDF').on('click', function(e) {
            ...
            var register_no = $('input[name="register_no"]').val();
            var session_id = $('select[name="session_id"]').val();

            // Create a temporary form for PDF download
            var tempForm = $('<form>', {
                'method': 'POST',
                'action': '<?php echo base_url("home/examResultsPDF"); ?>',
                'target': '_blank'
            });
            ...
            // Add form data
            tempForm.append($('<input>', {'type': 'hidden', 'name': 'exam_id', 'value': exam_id}));
            tempForm.append($('<input>', {'type': 'hidden', 'name': 'register_no', 'value': register_no}));
            tempForm.append($('<input>', {'type': 'hidden', 'name': 'session_id', 'value': session_id}));
