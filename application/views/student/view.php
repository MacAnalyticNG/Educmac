	...
	<div class="col-md-12">
		...
		<section class="panel appear-animation hidden-div" data-appear-animation="<?= $global_config['animations'] ?>" data-appear-animation-delay="100">
			<header class="panel-heading">
				...
				<?php endif; ?> <?php if (get_permission('student', 'is_edit')): ?>
					<div class="panel-btn">
						<a href="<?php echo base_url('student/bulk_disable_accounts'); ?>" class="btn btn-warning btn-circle">
							<i class="fas fa-ban"></i> <?= translate('bulk_disable_accounts') ?>
						</a>
					</div>
				<?php endif; ?> <h4 class="panel-title"><i class="fas fa-user-graduate"></i> <?php echo translate('student_list'); ?></h4>
			</header>
			...
			$.ajax({
				...
				success: function(data) {
					...
					if (data.status == "fail") {
						...
						cusDataTable = initDatatable("#studentTable", "student/getStudentListDT", data.filter, 25, true, false, [{
							"orderable": false,
							"targets": 'no-sort'
						}, {
							"class": 'center',
